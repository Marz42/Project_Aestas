...
| **功能伴随测试** | 每一项功能开发必须同时交付对应自动化测试 | 只提交业务代码、没有 pytest / 集成测试的 PR |
...
> **强制**：本项目的所有功能开发都必须伴随对应的自动化测试，与功能代码同一变更集交付。无测试的功能视为未完成。

## 适用范围

| 变更类型 | 最低测试要求 |
|----------|----------------|
| 后端 API 端点 | `pytest` + `httpx.AsyncClient` 集成测试（状态码、响应体、`ApiResponse` 结构） |
| Service / 业务逻辑 | 单元测试（可 mock 外部 IO：DB、RSS、LLM） |
| Celery 任务 | 任务函数单元测试；调度配置可用 smoke 测试或文档化手动验证 |
| 前端组件 / 页面（P1 起） | Vitest + Vue Test Utils（与后端同级要求） |
| Bug 修复 | 先写失败用例复现，再修复至通过 |

## 测试原则

- **同步交付**：不允许「先合功能、后补测试」；Agent 提交代码前必须运行 `pytest` 并确保通过。
- 修改已有代码时，先确保已有测试通过，再写新的测试。
- 不要为了覆盖率而写无意义的测试（比如断言框架本身的行为）。
- 测试应覆盖 happy path 与关键失败路径（如 404、校验错误）。
...
| 前端（P1） | Vitest、Vue Test Utils |
| 运行 | 本地：`cd backend && pytest`；CI：后续在 GitHub Actions 中强制执行 |

## 测试文件放置

- 后端：统一放在 `backend/tests/`，命名 `test_<模块>.py`
- 单元测试可与被测模块镜像子目录（如 `tests/services/ingestion/`）
- 前端：`*.test.ts` 与组件同目录或 `frontend/src/**/__tests__/`

## Agent 行为约定（测试）

- 新增或修改功能时，**同一 commit / PR** 内包含测试文件。
- 若某次变更确实无法自动化（如纯配置、一次性脚本），在 commit message 或 `progress.md` 中说明原因——默认仍需要测试。
- 提交前执行：`cd backend && pytest -q`（有前端变更时另跑 `npm test`）。
...
- [ ] 如果是新增功能，是否更新了相关文档
- [ ] **是否为新功能/修复添加了对应 pytest（或前端）测试，且本地已通过**
