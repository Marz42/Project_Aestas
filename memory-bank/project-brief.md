...
5. **Markdown 交付**：简报正文以 Markdown 存库；提供 REST 查询与下载（`GET /tag-briefs/{id}`、`/download`）。

### 后置（M5+）

- 飞书 / 企微 Webhook 推送
- Jina 全文补全、`reports/` 文件导出（文档曾标可选，**当前未实现**）
- 去重增强（MVP 仅有 URL 唯一）
...
Vue 3 · TypeScript · Vite · Element Plus · Axios（管理后台：控制台 / 信源 / 文章 / 简报 / Prompt）
...
| 优先级 | 功能 | 状态 |
|--------|------|------|
| P0 | 板块 tag + RSS 信源 | 已完成 |
| P0 | 8h 采集 Pipeline | 已完成 |
| P0 | DeepSeek 单条提炼 | 已完成 |
| P0 | 8h 窗口板块 Markdown 简报 | 已完成 |
| P0 | API 查看/下载简报 | 已完成 |
| P1 | Vue 管理后台 | 已完成（M4） |
| P1 | 基础去重（URL 唯一） | 已完成 |
| P1 | 去重增强 / 全文 Jina | M5 |
