...
| 字段 | 说明 |
|------|------|
| `content_tags` | **M5b-2**：1~3 个 taxonomy slug，提炼时 LLM 从预定义池多选；ANN 用 `&&` 交集召回 |

`short_news_md`：单条 Markdown 卡片。

### `taxonomy_tags`（M5b-2 计划）

约 20 个粗粒度大类（`slug`, `name_zh`）。详见 `domains/clustering-m5b2-plan.md`。

### `tags.taxonomy_slugs`（M5b-2 计划）

信源板块 → 默认 taxonomy 映射（辅助简报纳入，**不**作为 ANN 硬过滤）。
...
| `intro_md` | 周期导语（本窗内该板块整体综述，中文） |
| `content_md` | 完整 Markdown 报告（含导语 + 各事件节） |
| `status` | 契约枚举含 `draft`；**实现默认 `generated`**（无草稿流转） |

唯一约束：`(tag_id, window_start)`。

**简报纳入规则（M5b 前）**：`articles.fetched_at` ∈ 窗且 `status = extracted`。  
**M5b 后**：按 `story_clusters` 生成；`tag_brief_items` 可关联 `story_cluster_id`。

## `tag_briefs.content_md` 结构（M5c 目标）

```markdown
# {tag_name} 简报 · {window_start} — {window_end} (UTC)

> 共 {cluster_count} 个事件 · {article_count} 篇报道
...
> **当前实现（M5c）** 按 `story_cluster` 生成；无 cluster 时显示占位文案。

---

# M5 表结构（已落库，`004_m5_story_clusters`）

## `story_clusters`

| 字段 | 说明 |
|------|------|
| `tag_id` | 主归属（primary 成员信源板块）；**向量聚类不以之为 ANN 硬过滤** |
| `window_start` / `window_end` | 与简报窗对齐 |
| `title` / `summary` | 事件中文标题与摘要 |
| `article_count` | 成员数 |

## `story_cluster_articles`

`story_cluster_id` + `article_id` + `role`（`primary` \| `supporting`）+ 可选 `similarity`

## `articles` 扩展

`source_origin`: `rss` \| `crawl`；`story_cluster_id` 可空 FK

## `article_insights` 扩展（M5b-2）

| 字段 | 说明 |
|------|------|
| `content_tags` | `text[]`，1~3 taxonomy slug，GIN 索引 |
| `embedding` | `vector(1024)`，bge-m3，HNSW |
| `embedding_model` / `embedded_at` | 溯源 |

## `prompt_templates.purpose`

`extraction` | `clustering` | `briefing_intro` | `briefing_cluster` — **均须在管理后台可编辑**

## M5 API（已实现）

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/tasks/cluster-briefs` | 各 tag 同窗 LLM 聚类 |
| POST | `/tasks/seed-prompts` | 种子四类 Prompt |
| GET | `/story-clusters` | 分页列表，`?tag_id=` |
| GET | `/story-clusters/{id}` | 详情含成员文章 |
...
| POST | `/tasks/cluster-briefs` | 各 tag 事件聚类 |
...
| 2026-06-03 | **M5 规划**：`story_clusters`、简报导语+按事件多链接、`purpose` 扩展；推送移出 M5 |
| 2026-06-03 | **M5 实施**：`004` 迁移、聚类/简报服务、story-clusters API、前台聚类页与 Markdown 预览 |
